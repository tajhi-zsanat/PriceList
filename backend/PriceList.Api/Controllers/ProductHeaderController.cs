...
namespace PriceList.Api.Controllers
{
    ...
    [ApiController]
    public class ProductHeaderController(IUnitOfWork uow) : ControllerBase
    {
        [HttpPost]
        ...
        public async Task<IActionResult> Create([FromBody] List<HeaderInputDto> dto, CancellationToken ct = default)
        {
            ...
            var existingPairs = await uow.ProductHeader
            ...
            // Save
            await uow.ProductHeader.AddRangeAsync(toInsert, ct);
            try
            {
